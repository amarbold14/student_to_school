...
const CustomMapDistToSchool = ({
  geoJsonRef,
  data,
  setFeatures,
  trafficData,
  selectedFeatures,
  selectedFeaturesBySchool,
  setData,
  title,
}) => {
  ...
  useEffect(() => {
    ...
    if (selectedFeaturesBySchool.length === 0) {
      ...
      tempStudentData.forEach((s) => {
        ...
        let sumHouseHold = 0;
        let sumHouseHoldWithoutCar = 0;
        selectedFeaturesBySchool.forEach((sm) => {
          let temp = district_to_district_flow.filter(
            (d) =>
              d?.school_khoroo_id?.toString() === sm?.properties?.id?.toString()
          );
          let a = temp.filter(
            (d) => d.home_khoroo_id?.toString() === s.properties?.id?.toString()
          );
  ...
  return (
    <div className="relative w-full h-full">
      ...
      >
        ...
        <div className="flex justify-center mt-2">
          <div>
            <LegendItems
              data={
                selectedFeaturesBySchool.length > 0
                  ? LegendItemsDistrictLocal
                  : LegendItemsDistrictGlobal
              }
            />
      ...
      <button
        className={
          "absolute bottom-4 left-4 py-2 px-6 bg-white opacity-80 shadow-2xl"
        }
        style={{ zIndex: 1000 }}
        onClick={() => {
          setModalOpen(true);
      ...
      >
        ...
        {/*<TileLayer url="https://khms0.google.com/kh/v=959?x={x}&y={y}&z={z}" />*/}
        ...
        {centers?.map((center, index) => {
          ...
          let studentCount = p?.total_students;
              ...
              >
                <Popup>
                  <div className="text-xs font-normal grid  gap-1 ">
                    ...
                    <div>
                      <span>Автомашингүй өрхийн тоо:</span>
                      <span className={"ml-4"}>
                        {p.student_flow_carless || 0}
                      </span>
                    </div>
                    <div className="flex justify-between">
...
export default CustomMapDistToSchool;
